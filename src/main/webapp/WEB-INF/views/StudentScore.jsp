...
<head>
    <title>学生成绩</title>
<%--    <meta http-equiv="refresh" content="0;url=${pageContext.request.contextPath}/StudentScore?pageStart=0&studentId=${studentId}"/>--%>
...
<div>
    <form action="${pageContext.request.contextPath}/StudentScore?pageStart=0&studentId=${studentId}" method="post">
        <tr>
...
<nav class="pageDIV">
    <ul style="list-style-type:none;margin:0;padding:0;">
        ...
        <li style="margin:0 10px;float: left;"<c:if test="${!page.hasPrevious}">class="disabled"</c:if>>
            <form action="${pageContext.request.contextPath}/StudentScore?pageStart=${page.start-page.count}&studentId=${studentId}" method="post">
                <input type="submit" value="‹">
            </form>
            </a>
        </li>
            ...
            <c:if test="${status.count*page.count-page.start<=30 && status.count*page.count-page.start>=-10}">
                <li style="margin:0 10px;float: left;"<c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                    <a <c:if test="${status.index*page.count==page.start}">class="current"</c:if> >
                        <form action="${pageContext.request.contextPath}/StudentScore?pageStart=${status.index*page.count}&studentId=${studentId}" method="post">
        ...
        <li style="margin:0 10px;float: left;"<c:if test="${!page.hasNext}">class="disabled"</c:if>>
            <form action="${pageContext.request.contextPath}/StudentScore?pageStart=${page.last}&studentId=${studentId}" method="post">
                <input type="submit" value="»">
            </form>
        </li>
    </ul>
